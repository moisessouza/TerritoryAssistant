...
/**
 * An {@link UseCase} for sending a volume down command to the MPC-HC server. New volume value depends on user settings
 * and is taken from {@link ged.mediaplayerremote.domain.repository.UserPreferencesRepository}
 */
public class SendVolumeDownCommand extends UseCase {

    private CommandDispatcher commandDispatcher;

    @Inject
    public SendVolumeDownCommand(ThreadExecutor threadExecutor, PostExecutionThread postExecutionThread,
    ...
    protected Observable buildUseCaseObservable() {
        return commandDispatcher.lowerVolume();
    }
}
